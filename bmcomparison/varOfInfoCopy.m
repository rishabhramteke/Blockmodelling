function [distance] = varOfInfoCopy(vMembership1, vMembership2)
%
...
% loop through each vertex in the membership vectors, and compare their
% labels
for v = 1 : size(vMembership1,dim)
    % need to use map to avoid the case where some cluster labels are
    % non-continous from [1..].
    rvVal1 = vSymbolMap1(vMembership1(v));
    rvVal2 = vSymbolMap2(vMembership2(v));
    mInterDistrib(rvVal1, rvVal2) = mInterDistrib(rvVal1, rvVal2) + 1;
end

% normalise the entries
mInterDistrib = mInterDistrib ./ size(vMembership1, dim);
...
vDistribution = vDistribution ./ size(vElems,dim); 
