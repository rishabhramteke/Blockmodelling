function [mAdjMat, vVertRole, mShuffledAdjMat, vShuffledVertRole] = genWeightedBlockmodel(sRoleDist, roleNum, graphSize, bShuffle, sGen, varargin)
%
...
% bShuffle - whether to shuffle the generated blockmodels
...
if optargin > 0
    if strcmp(sGen, 'gen')
        ...
        meanMeanDense = varargin{2};
        varMeanDense = varargin{3};
        meanVarDense = varargin{4};
        varVarDense = varargin{5};
        meanMeanSparse = varargin{6};
        varMeanSparse = varargin{7};
        meanVarSparse = varargin{8};
        varVarSparse = varargin{9};
        currIndex = currIndex + 9;
        [mImageMean, mImageStd] = genImageMatrix(probDense, blockNum, meanMeanDense, varMeanDense, meanVarDense, varVarDense,...
            meanMeanSparse, varMeanSparse, meanVarSparse, varVarSparse);
    else
        % read from arguments
        mImageMean = varargin{1};
        currIndex = currIndex + 1;
        mImageStd = varargin{2};
        currIndex = currIndex + 1;
    end
...
assert(size(vRoleProb,2) == size(mImageMean,2));
assert(size(mImageMean,1) == size(mImageMean,2));
assert(size(mImageStd, 1) == size(mImageStd,1));
...
function [mImageMean, mImageStd] = genImageMatrix(probDense, blockNum, meanMeanDense, stdMeanDense, meanStdDense, stdStdDense,...
            meanMeanSparse, stdMeanSparse, meanStdSparse, stdStdSparse);
%
...
mImageMean = zeros(blockNum, blockNum);
mImageStd = zeros(blockNum, blockNum);
mDense = rand(blockNum, blockNum);

for r = 1 : blockNum
    for c = 1 : blockNum
        if mDense(r,c) < probDense
            mean = normrnd(meanMeanDense, stdMeanDense);
            if mean < 0.0, mean = 0.0; end
            mImageMean(r,c) = mean;
            
            std = normrnd(meanStdDense, stdStdDense);
            if std < 0.0, std = 0.0; end
            mImageStd(r,c) = std;            
        else
            mean = normrnd(meanMeanSparse, stdMeanSparse);
            if mean < 0.0, mean = 0.0; end
            mImageMean(r,c) = mean;
            
            std = normrnd(meanStdSparse, stdStdSparse);
            if std < 0.0, std = 0.0; end
            mImageStd(r,c) = std;     
        end
    end
