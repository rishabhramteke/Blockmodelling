function [mAdjMat, vVertRole, mShuffledAdjMat, vShuffledVertRole] = genBlockmodelDeg(sRoleDist, roleNum, graphSize, bShuffle, plantedProp, sGen, varargin)
%
...
% plantedProp - (newman) The percentage that the matrix is generated from
% planted data.
...
if stdargin < 5
    disp(' genBlockmodel(vRoleProb, graphSize, bShuffle, sGen, platendProp, varargin)');
    exit;
end
...
switch sImageGen
    ...
    case 'poissonImage'
        % from block generate the number of edges (poisson(lambda), lambda =
        % rate
        
        % generate expected degrees for the vertices
        % P(y) = (y+b)^(-n)
        vX = rand(1, graphSize);
        vExpectedDeg = vX.^(1/1-n) - b;
        
    otherwise
        error('sImageGen %s is unknown', sImageGen);
end
