classdef ImageMultBM
    ...
    methods
        ...
        function obj = ImageMultBM(varargin)
        %
        % INPUT:
        % varargin{1}:
        % varargin{2}:
        ...
        %
            ...
            % need to separate into positive and negative parts
            mNegGrad = mRegGraD;
            mNegGrad(mNegGrad > 0) = 0;
            mPosGrad = mRegGraD;
            mPosGrad(mNegGrad < 0) = 0;
            
            % update image
            mPos = mMembership' * mAdj * mMembership + fDistanceFunc.m_regPara * mPosGrad + obj.m_paraL1MembershipReg + obj.m_paraL2MembershipReg * mImage;
            mNeg = mXtX * mImage * mXtX - fDistanceFunc.m_regPara * mNegGrad;
            
            
            % update
            mImage = mImage .* ((mNeg) ./ (mPos + eps))^0.25;
