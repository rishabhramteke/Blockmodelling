function plotColouredSingleBlockmodel(mSnapshot, mVertPosMap, mPosColour, sTitle, bUseVertIndices, bPosSort)
%
...
for currPart = 1 : partNum
    ...
    if bUseVertIndices
        drawColouredBlockmodel(mSnapshot(vI, vI), mVertPosMap(vI,:) ,mPosColour, vBoundary, vBoundary, vI, vI); 
    else
        drawColouredBlockmodel(mSnapshot(vI, vI), mVertPosMap(vI,:) ,mPosColour, vBoundary, vBoundary); 
    end
